...
df2 <- df2 %>%
  mutate(
    gift_size_category = case_when(
      ...
      donation_amount >= 1000 ~ "Leadership ($1,000+)",
      TRUE ~ "Unknown"
    ),
    gift_size_category = factor(gift_size_category,
                                levels = c("<$20", "$20-$50", "$50-$100", "$100-$200",
                                           "$200-$500", "$500-$1,000", "Leadership ($1,000+)", "Unknown"))
  )
...
df2 <- df2 %>%
  mutate(
    ...
    fiscal_year = case_when(
      month(capture_date) >= 6 ~ calendar_year + 1,
      TRUE ~ calendar_year
    )
  )
...
df2 <- df2 %>%
  mutate(
    academic_year = case_when(
      month(capture_date) >= 7 ~ calendar_year + 1,
      TRUE ~ calendar_year
    )
  )
...
df2 <- df2 %>%
  mutate(
    freshman_year_limit = academic_year + 3,
    parent_tag = case_when(
      (parent_year >= academic_year) & (parent_year <= freshman_year_limit) ~ "Current Parent",
      parent_year < academic_year ~ "Past Parent",
      parent_year > (freshman_year_limit + 5) ~ "Future Parent (Likely Typo)", 
      parent_year > freshman_year_limit ~ "Future Parent",
...
df2 <- df2 %>%
  ...
  # ONLY match if we have the same email AND same name
...
write.csv(df2, "homepagedata.csv", row.names = FALSE, na = "")


max(df2$capture_date)
df2_clean <- df2 %>%
...
max(df2$fiscal_year)
# Should be <= "2025-05-31" if no FY 2026 data
# Should be <= "2025-05-31" if no FY 2026 data